...
export async function DELETE(req: Request, context: { params: Promise<{ id: string, memberId: string }> }) {
  try {
    ...
    team.members = team.members.filter((m: any) => m.toString() !== params.memberId);
    await team.save();
